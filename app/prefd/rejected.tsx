...
const Rejected = () => {
  ...
  const filteredForms = submittedForms.filter((item) => {
    ...
    const isPending = item.formStatus === "Rejected";
  ...
  const handleCardPress = (item) => {
    ...
    router.push({ pathname: previewPath, params: { id: item.id, fromsubmit: "true", returnsubmit: "/prefd/rejected" } });
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {filteredForms.length === 0 ? (
        <Text style={styles.noDataText}>No Rejected forms found.</Text>
      ) : (
